...
class course_renderer extends \core_course_renderer {
    ...
    public function course_section_cm($course, &$completioninfo, cm_info $mod, $sectionreturn, $displayoptions = array()) {
        ...
        if ($actions) {
            ...
            if($mod->modname == 'quiz'){
              echo '<style>.snap-asset-actions>a, .snap-asset-actions .snap-edit-more-dropdown, .snap-asset-actions .snap-asset-move {
                width:25%;
                padding:0;
              }</style>';
              $url = new moodle_url('/mod/quiz/edit.php', ['cmid' => $mod->id]);
              // $addalt = s(get_string('add', 'theme_snap', $mod->get_formatted_name()));
              $addicon = '<img title="Add Quiz Questions" alt="Add Quiz Questions" class="svg-icon" src="';
              $addicon .= $this->output->image_url('add', 'theme').'"/>';

              $output .= '<a href="'.$url.'">'.$addicon.'</a>';
            }
