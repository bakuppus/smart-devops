...
class auth_plugin_userkey extends auth_plugin_base {
    ...
    public function logoutpage_hook() {
        ...
        if (!filter_var($this->config->wpurl, FILTER_VALIDATE_URL)) {
            return true;
        }

        $redirect = strtok($this->config->wpurl, '?') .'?oneacademyssoaction=logout&mdl_user_id='.$USER->id.'&redirect_to='.strtok($this->config->wpurl, '?');
    }
...
//         Guest user.
        ...
        $wpsiteurl = strtok($this->config->wpurl, '?');
        $SESSION->wantsurl = $CFG->wwwroot.'/auth/userkey/wp-login.php?'.'oneacademyssoaction=login&mdl_user_id='.$user->id.'&wpsiteurl='.urlencode($wpsiteurl);
        return true;
    }
}
