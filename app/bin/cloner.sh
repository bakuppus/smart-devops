...
#clone default DB
    mysql --host=$DB_HOST --user=$DB_USER --password=$DB_PASS <<< "USE $DB_NAME;set global innodb_large_prefix=on; set global innodb_file_format=Barracuda"

#echo $DB_user
echo "Step 2 -- Import Default Moodle DB to $DB_NAME"
mysql --host=$DB_HOST --user=$DB_USER --password=$DB_PASS $DB_NAME < lms_default_db.sql
...
#update SSO url
echo "Step 5 -- Set SSO Links [$LMS_NAME]"
...
mysql --host=$DB_HOST --user=$DB_USER --password=$DB_PASS  $DB_NAME -e "UPDATE mdl_config SET value = '$SSOURL/wp-login.php' WHERE name = 'alternateloginurl';"
mysql --host=$DB_HOST --user=$DB_USER --password=$DB_PASS  $DB_NAME -e "UPDATE mdl_config SET value = '$SSOURL/wp-login.php?action=lostpassword' WHERE name = 'forgottenpasswordurl';"
...
echo "Step 6 -- Create Moodle DataRoot Folder"
...
echo "Step 7 -- Update Moodle DataRoot folder permissions"
