...
final class OneCreate{
    ...
    public function __construct() {
        ...
        require_once plugin_dir_path( __FILE__ ) . 'class/class-remove-tenant-data.php';
        ...
        $this->remove_tenant_data = new OA_Remove_Tenant_Data();
        ...
        */
        ...
        /*
        *  Remove Blog/Site files after removal
        *  @author: MouMohsen
        */
        add_action( 'delete_blog', array($this,"clear_subsite_files_database"), 10, 2 );
...
//            ) );
        ...
        add_action( 'init', function(){
            add_rewrite_endpoint( 'lms', EP_ROOT );
        } );

        add_action( 'template_redirect', function(){
            if($machinesUrl = get_query_var('lms')){
                wp_redirect($this->OneAcademySSO->get_mdl_login_url(get_current_user_id(),'/'));
                die();
            }
        });

        add_filter( 'request', function($vars=[]){
            if(isset ( $vars['lms'] ) && empty ( $vars['lms'] )){
                $vars['lms'] = 'default';
            }
            return $vars;
        });
        //lms-admin link
        add_action( 'init', function(){
            add_rewrite_endpoint( 'lms-admin', EP_ROOT );
        } );

        add_action( 'template_redirect', function(){
            if($machinesUrl = get_query_var('lms-admin')){
                wp_redirect($this->OneAcademySSO->get_mdl_login_url(get_current_user_id(),'/admin/search.php'));
                die();
            }
        });

        add_filter( 'request', function($vars=[]){
            if(isset ( $vars['lms-admin'] ) && empty ( $vars['lms-admin'] )){
                $vars['lms-admin'] = 'default';
            }
            return $vars;
        });

        //lms-manage-courses link
        add_action( 'init', function(){
            add_rewrite_endpoint( 'lms-manage-courses', EP_ROOT );
        } );

        add_action( 'template_redirect', function(){
            if($machinesUrl = get_query_var('lms-manage-courses')){
                wp_redirect($this->OneAcademySSO->get_mdl_login_url(get_current_user_id(),'/course/management.php'));
                die();
            }
        });

        add_filter( 'request', function($vars=[]){
            if(isset ( $vars['lms-manage-courses'] ) && empty ( $vars['lms-manage-courses'] )){
                $vars['lms-manage-courses'] = 'default';
            }
            return $vars;
        });
    }
    ...
    public function lms_admin_bar_menu() {
        ...
        if(!get_blog_option($blog_id,'lms_built') && $blog_id !=1 ){
            ...
            $mdlUrl=$this->OneAcademySSO->get_mdl_login_url(get_current_user_id());
    ...
    */
    public function addNetworkNewSite(){
        ...
        if( isset( $_POST['nds_add_user_meta_nonce'] ) && wp_verify_nonce( $_POST['nds_add_user_meta_nonce'], 'nds_add_user_meta_form_nonce') ) {
            ...
            //Create New Site & Assign Admin EMAIL as admin
            $id = wpmu_create_blog( $newdomain, $path, $title, $user_id, $meta, get_current_network_id() );

            //Assign Admin USER to network
            add_user_to_blog($id , $user_id, 'shop_manager');
    ...
    */
    ...
    public function filter_network_users_page_callback() {
      ...
      */
      public function clear_subsite_files_database( $blog_id, $drop ){
        $this->remove_tenant_data->data( array( 'id' => $blog_id, 'lms_prefix'=>$this->lms_prefix ) )->dispatch();
      }
