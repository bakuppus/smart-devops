  ...
  */
  function extra_tablenav( $which ) {
    if ( $which == "top" ){
      ...
      echo '<style>.tablenav.top{height:auto !important;}.selectList {display: flex;align-items: center;width: 25%;justify-content: space-between;}</style>';
        ...
        foreach ( $columns as $column_name => $column_display_name ) {
          ...
          switch ( $column_name ) {
            ...
            if (is_super_admin($rec->ID)) {
              echo '<td '.$attributes.'><a href="'.network_admin_url().'user-edit.php?user_id='.$rec->ID.'"><b>'.stripslashes($rec->user_login).'</b></a> - Super Admin </td>';
            } else {

              echo '<td '.$attributes.'><a href="'.network_admin_url().'user-edit.php?user_id='.$rec->ID.'">'.stripslashes($rec->user_login).'</a></td>';
            }
