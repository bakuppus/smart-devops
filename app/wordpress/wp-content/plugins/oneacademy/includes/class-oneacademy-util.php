...
class OneAcademyUtil {
    ...
    public static function create_mdl_user($wp_user_id, $args = array()) {
        ...
        if (isset($response[0]['id'])) {
            ...
            self::assign_manual_user_roles($wp_user_id,$response[0]['id']);
    ...
    public static function update_mdl_user($wp_user_id, $args=array()) {
        ...
        if (!empty($exist_user)) {
            ...
            $moodle_user_id = $exist_user['id'];
            self::assign_manual_user_roles($wp_user_id, $moodle_user_id);
            update_user_meta($wp_user_id, '_mdl_user_id', $exist_user['id']);
        }
    }
    ...
    public static function create_moodle_course($post_id) {
        ...
        if ($product) {
            ...
            if (isset($response[0]['id'])) {
                ...
                $moodle_user = self::get_mdl_user_by_field($_SESSION['oa_current_user_id']);
                self::enrol_manual_enrol_users($moodle_user['id'], $response[0]['id']);
...
//                }
    ...
    public static function assign_manual_user_roles($wp_user_id, $mdl_user_id) {
      if (is_null(oa()->mdl_site_url) || is_null(oa()->mdl_access_token) || !oa()->is_connected) {
        return;
      }
      ...
      foreach ($user_roles as $user_role) {
        if ($user_role == 'shop_manager') {
          $serverurl = oa()->mdl_site_url . '/webservice/rest/server.php' . '?wstoken=' . oa()->mdl_access_token . '&wsfunction=core_role_assign_roles&moodlewsrestformat=' . OA_REST_FORMAT;
          $key = 'assignments';
        } else {
          //make sure user not assigned to manager roles
        $serverurl = oa()->mdl_site_url . '/webservice/rest/server.php' . '?wstoken=' . oa()->mdl_access_token . '&wsfunction=core_role_unassign_roles&moodlewsrestformat=' . OA_REST_FORMAT;
        $key = 'unassignments';
        }

        $manager_roles_ids = [1,14];
        foreach ($manager_roles_ids as $manager_roles_id) {
          $assign_user_role = new stdClass();
          $assign_user_role->roleid = $manager_roles_id;
          $assign_user_role->userid = $mdl_user_id;
          $assign_user_role->contextid = 1;
          $params = array($key => array($assign_user_role));
          oa()->curl->post($serverurl, $params);
        }
      }
    }
