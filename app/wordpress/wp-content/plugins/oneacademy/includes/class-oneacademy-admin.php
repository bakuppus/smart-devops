...
//        add_action('edit_user_profile', array($this, 'add_customer_meta_fields'));
    ...
    private function init_hooks() {
        add_action('init', array($this, 'oa_current_user_id'));
    }

    public function oa_current_user_id(){
      $_SESSION['oa_current_user_id'] = get_current_user_id();
    }

    public function connection_problem_notice(){
        if(!oa()->is_connected){
            echo '<div class="error"><p>';
            _e( 'This site is not connected with LMS', 'woo-wallet' );
            echo '</p></div>';
        }
    }
...
//        add_submenu_page('oneacademy', 'Oneacademy Courses', 'Oneacademy Courses', 'manage_woocommerce', 'oneacademy-courses', array($this, 'oneacademy_courses'));
        add_submenu_page('oneacademy', _('Create OneAcademy Course'), _('Create OneAcademy Course'), 'manage_woocommerce', 'post-new.php?post_type=product&product-type=mdl_course');
    }
    ...
    public function manage_users_columns($columns) {
        if (current_user_can('manage_woocommerce')) {
            $columns['mdl_account'] = __('LMS Account', 'woo-wallet');
        }
        return $columns;
    }
    ...
    public function manage_users_custom_column($value, $column_name, $user_id) {
        if ($column_name === 'mdl_account') {
            if (!get_user_meta($user_id, '_mdl_user_id', true)) {
                ...
                $value = '<a href="' . $url . '" class="button">' . __("Sync") . '</a>';
            } else {
                $url = wp_nonce_url(get_edit_user_link($user_id), 'do_create_mdl_user', 'do_create_mdl_user');
                $value = '<a href="' . $url . '" class="button">' . __('Re-Sync') . '</a>';
            }
        }
        return $value;
    }
    ...
    public function post_submitbox_misc_actions() {
        ...
        <div id="moodle_course_url" class="moodle_course_url misc-pub-section">
            ...
            <a class="button button-primary" target="__blank" href="<?php echo OneAcademySSO::get_mdl_login_url(get_current_user_id(), oa()->mdl_site_url . '/course/view.php?id=' . get_post_meta($post->ID, '_mdl_course_id', true)); ?>"><?php echo __('Edit Course'); ?></a>
            <?php } else { ?>
    ...
    public function add_customer_meta_fields($user) {
        ...
        $enrolled_courses_ids = wp_list_pluck($enrolled_courses ? $enrolled_courses : array(), 'id');
        ?>
