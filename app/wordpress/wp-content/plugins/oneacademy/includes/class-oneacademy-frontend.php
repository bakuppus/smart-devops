...
class OneAcademyFrontend {
    ...
    public static function moodle_dashboard_shortcode_output() {
        if(oa()->is_connected){
            //printf('<a class="button" href="%1$s" target="__blank">Go to LMS</a>', OneAcademySSO::get_mdl_login_url(get_current_user_id()));
            printf('<a class="button" href="%1$s" target="__blank">Go to LMS</a>', '/lms');
        } else{
            printf('<button type="button" disabled class="button">Go to LMS</button>');
        }
    }
    ...
    public function woocommerce_order_item_meta_end($item_id, $item, $order) {
        ...
        if ($courseid && $userid) {
            ...
            echo sprintf('<p>Course: <a href="%s" target="__blank">%s</a></p>', $mdl_course_link_with_login, $product->get_title());
        }
    }
    ...
    public function woocommerce_account_dashboard() {
        if(oa()->is_connected){
            ...
            printf('<button type="button" disabled class="button"></button>');
        }
    }
