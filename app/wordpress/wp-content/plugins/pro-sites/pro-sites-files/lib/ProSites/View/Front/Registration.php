...
if ( ! class_exists( 'ProSites_View_Front_Registration' ) ) {
	class ProSites_View_Front_Registration {
		...
		private static function render_blog_section( $render_data = array(), $errors, $blogname = '', $blog_title = '' , $blog_country = '' , $blog_zip = '') {
			$current_site = get_current_site();
			$content = '<div>';
...
//			}
            ...
            $content .= '<div class="blog_country"><label for="blog_country">' . esc_html__('Country:', 'psts' ) . '</label>';
            if ( $errmsg = $errors->get_error_message('blog_country') ) {
                $content .= '<p class="error">' . $errmsg . '</p>';
            }
            $content .= '<input name="blog_country" type="text" id="blog_country" value="'.esc_attr( $blog_country ) . '" /></div>';

            /* Add Zip Input Field */
            $content .= '<div class="blog_zip"><label for="blog_zip">' . esc_html__('Zip Code:', 'psts' ) . '</label>';
            if ( $errmsg = $errors->get_error_message('blog_zip') ) {
                $content .= '<p class="error">' . $errmsg . '</p>';
            }
            $content .= '<input name="blog_zip" type="text" id="blog_zip" value="'.esc_attr( $blog_zip ) . '" /></div>';
