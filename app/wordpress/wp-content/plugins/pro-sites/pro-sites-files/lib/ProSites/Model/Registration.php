	...
	class ProSites_Model_Registration {
		...
		public static function ajax_check_prosite_blog() {
			...
			if ( $doing_ajax ) {
				...
				$blog_country = sanitize_text_field( urldecode( $_POST['blog_country'] ) );
				$blog_zip     = sanitize_text_field( urldecode( $_POST['blog_zip'] ) );

				// Process some cleaning up if needed
				do_action( 'prosite_register_blog_pre_validation', $user_name, $user_email, $blogname );

				$blog_validation = wpmu_validate_blog_signup( $blogname, $blog_title ,$blog_country , $blog_zip);
